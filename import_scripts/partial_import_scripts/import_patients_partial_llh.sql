...
DROP PROCEDURE IF EXISTS `proc_import_patients_partial_llh`$$

# Procedure does not take any parameters. It assumes fixed table names and database
# names as working with flexible names is not supported as of writing in MySQL.
CREATE PROCEDURE `proc_import_patients_partial_llh`(
  #--IN start_pos INT(11),
  #--IN end_pos INT(11)
...
#--check if patient already exist
        SET @patient_exists = COALESCE((SELECT p.patient_id FROM openmrs_llh.patient p WHERE p.patient_id = patient_id ), 0);
