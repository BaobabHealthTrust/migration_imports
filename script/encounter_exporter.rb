...
def start
  ...
  encounters_to_migrate = staging + art_visit

  #you can specify the number of patients to export by adding limit then number of patiets e.g limit 100 to the query below
  patients = Patient.find_by_sql("Select * from #{Source_db}.patient where voided = 0 AND patient_id in
                                (SELECT distinct patient_id from encounter where encounter_id in (#{encounters_to_migrate.join(',')}))")
  ...
  patients.each do |patient|
    ...
    encounters = Encounter.find_by_sql("Select e.* from #{Source_db}.encounter e
                                        where e.patient_id = #{patient.id} and e.encounter_id
                                        IN (#{encounters_to_migrate.join(',')})")
