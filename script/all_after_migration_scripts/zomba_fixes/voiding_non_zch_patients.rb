...
def start
  ...
  non_zch_arv_ids_patients = Encounter.find_by_sql("SELECT * FROM openmrs_zch_bart1.patient_identifier
  ...
  (non_zch_arv_ids_patients || []).each do |patient|
    ...
    pat_encs = []
    pat_encs = Encounter.find_by_sql("select e.* from openmrs_zch_bart1.encounter e
                                        inner join openmrs_zch_bart1.obs o on o.encounter_id = e.encounter_id and o.voided = 0
                                      where o.patient_id = #{patient.patient_id} GROUP BY e.encounter_id
                                      union all
                                      select e.* from openmrs_zch_bart1.encounter e
                                        inner join openmrs_zch_bart1.orders o on o.encounter_id = e.encounter_id and o.voided = 0
