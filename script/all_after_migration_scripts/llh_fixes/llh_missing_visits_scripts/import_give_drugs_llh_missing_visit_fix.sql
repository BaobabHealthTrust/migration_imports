...
DROP PROCEDURE IF EXISTS `proc_import_give_drugs_llh_missing_visit`$$

# Procedure does not take any parameters. It assumes fixed table names and database
# names as working with flexible names is not supported as of writing in MySQL.
CREATE PROCEDURE `proc_import_give_drugs_llh_missing_visit`(
...
`bart1_intermediate_bare_bones`.`give_drugs_encounters` 
       ...
       AND `bart1_intermediate_bare_bones`.`give_drugs_encounters`.`encounter_datetime` = in_encounter_datetime;
