...
DROP PROCEDURE IF EXISTS `proc_import_hiv_reception_encounters_llh_missing_visit`$$

# Procedure does not take any parameters. It assumes fixed table names and database
# names as working with flexible names is not supported as of writing in MySQL.
CREATE PROCEDURE `proc_import_hiv_reception_encounters_llh_missing_visit`(
...
`bart1_intermediate_bare_bones`.`hiv_reception_encounters`.`encounter_datetime`, `bart1_intermediate_bare_bones`.`hiv_reception_encounters`.`date_voided`, `bart1_intermediate_bare_bones`.`hiv_reception_encounters`.`voided_by`,  `bart1_intermediate_bare_bones`.`hiv_reception_encounters`.`date_created`, `bart1_intermediate_bare_bones`.`hiv_reception_encounters`.`creator`, COALESCE(`bart1_intermediate_bare_bones`.`visit_encounters`.visit_date, `bart1_intermediate_bare_bones`.`hiv_reception_encounters`.date_created) FROM `bart1_intermediate_bare_bones`.`hiv_reception_encounters` LEFT OUTER JOIN `bart1_intermediate_bare_bones`.`visit_encounters` ON
        ...
        AND `bart1_intermediate_bare_bones`.`hiv_reception_encounters`.`encounter_datetime` = in_encounter_datetime;
...
DECLARE CONTINUE HANDLER FOR NOT FOUND SET done = TRUE;
  ...
  SET @encounter_id = (SELECT LAST_INSERT_ID());
        ...
        IF NOT ISNULL(guardian) THEN
            ...
            VALUES (patient_id, @guardian_concept_id, @encounter_id, encounter_datetime, @location_id , @guardian_value_coded, @guardian_value_coded_name_id, @creator, date_created, (SELECT UUID()));
        ...
        IF NOT ISNULL(patient_present) THEN
            ...
            INSERT INTO obs (person_id, concept_id, encounter_id, obs_datetime, location_id , value_coded, value_coded_name_id, creator, date_created, uuid)
            VALUES (patient_id, @patient_present_concept_id, @encounter_id, encounter_datetime, @location_id , @patient_present_value_coded, @patient_present_value_coded_name_id, @creator, date_created, (SELECT UUID()));
        ...
        IF NOT ISNULL(guardian_present) THEN
            ...
            # Create observation
            INSERT INTO obs (person_id, concept_id, encounter_id, obs_datetime, location_id , value_coded, value_coded_name_id, creator, date_created, uuid)
            VALUES (patient_id, @guardian_present_concept_id, @encounter_id, encounter_datetime, @location_id , @guardian_present_value_coded, @guardian_present_value_coded_name_id, @creator, date_created, (SELECT UUID()));
