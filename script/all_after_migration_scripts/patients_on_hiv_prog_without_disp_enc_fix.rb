...
def start
  ...
  (patients || []).each do |patient|
    ...
    patient_treatment_encounter = Encounter.find_by_sql("  SELECT patient_id
                                                            ...
                                                            AND encounter_type = 54 ").map(&:encounter_datetime)
...
EOF
=end
         puts"voiding HIV program and states of patient_id: #{patient}"                                   
      else
=begin
ActiveRecord::Base.connection.execute <<EOF
...
EOF
=end
        puts"updating ON ARVs state to Pre-ART state of patient_id: #{patient}"                                      
      end
    end
