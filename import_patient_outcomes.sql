...
BEGIN
    ...
    IF NOT ISNULL(patient_id) THEN
      ...
      IF (@patient_hiv_program_id = 0) THEN
        # create patient_program
          ...
          IF NOT ISNULL(state) THEN
            ...
            ELSEIF state = 'Defaulter' THEN
              # create update_outcome_encounter and outcome observation 
              select patient_id;
            ELSE
              select patient_id;
            END IF;
      ...
      ELSE #patient_program
        ...
        IF (state = 'Died' OR state = 'ART Stop' OR state = 'Transfer Out(With Transfer Note)' OR state = 'Transfer Out(Without Transfer Note)' OR state = 'Transfer out') THEN
          ...
          IF state = 'Died' THEN
            ...
            WHERE person_id = patient_id and voided = 0;
         ...
         VALUES (patient_id, @reason_for_existing_care_concept_id, @new_encounter_id, outcome_date, @reason_value_coded, @reason_value_coded_name_id, 1,(SELECT UUID()));
          ...
          #--       IF voided != 1 THEN
          #--            UPDATE patient_program SET date_completed = date_created WHERE patient_program_id = @patient_hiv_program_id;
          #--        END IF;
